...
"""
    ...
    Types supported include JSON base types, typed collections from the `typing` module, literal types, union types,
...
def from_json_obj(obj: Any, t: Any, cast_decimal: bool = True) -> Any:
    """
        ...
        - if `t` is one of the JSON base types `bool`, `float`, `str`, `NoneType`, `obj` must be an instance of the type an is returned unchanged;
        - if `t` is of the JSON base type `int` and the `cast_decimal` parameter is set to `False`, `obj` must be an instance of `int` and is returned unchanged;
        - it `t` is of the JSON base type `int` and the `cast_decimal` parameter is set to `True` (its default value), `obj` can be either an instance of `int`, in which case it is returned unchanged, or an instance of `decimal.Decimal` encoding an integer, in which case `obj.to_integral_value()` is returned;
        - if `t` is of the JSON base type `float` and the `cast_decimal` parameter is set to `False`, `obj` must be an instance of `float` and is returned unchanged;
        - it `t` is of the JSON base type `float` and the `cast_decimal` parameter is set to `True` (its default value), `obj` can be either an instance of `float`, in which case it is returned unchanged, or an instance of `decimal.Decimal`, in which case `float(obj)` is returned;
        - if `t` is `None`, used as an alias for `NoneType`, `obj` must be `None` and is returned unchanged;
        - if `t` is `decimal.Decimal`, `obj` must be either an `int` or a `str` encoding a valid decimal, in which case `decimal.Decimal(obj)` is returned;
    ...
    if t in JSON_BASE_TYPES:
        # JSON base types are returned unaltered, with the exception of casting `Decimal` to `int`/`float` if `cast_decimal` is `True`.
    ...
    if t in (None, type(None)):
        ...
        return None
    if t == Decimal:
        # Instances of `decimal.Decimal` are decoded from either `int` of `string`
        try:
            if isinstance(obj, (int, str)) and obj is not True and obj is not False:
                return Decimal(obj)
